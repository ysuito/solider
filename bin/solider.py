...
parser.add_argument('command', choices=[
                    'start', 'build', 'build-all', 'update', 'entry', 'list'], help='command')
parser.add_argument('app_name', help='app name', nargs='?')

args = parser.parse_args()

home = os.environ['HOME']
soliderhome = os.path.abspath(os.path.join(
...
base_command = [
    ...
    '-e', 'PULSE_COOKIE=/run/user/{}/pulse/cookie'.format(uid),
    '-e', 'PULSE_SERVER=unix:/run/user/{}/pulse/native'.format(uid),
    '-e', 'DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/{}/bus'.format(
        uid),
    '-v', '/run/user/{0}/pulse/native:/run/user/{0}/pulse/native'.format(uid),
    '-v', os.path.join(home,
                       '.config/pulse/cookie:/run/user/{}/pulse/cookie:ro'.format(uid)),
    '-v', '/tmp/.X11-unix/X0:/tmp/.X11-unix/X0',
    '-v', '/run/user/{0}/bus:/run/user/{0}/bus'.format(uid),
    '-u', 'user',
]
...
class Solider():
    ...
    def build_all(self):
        ...
        base_images = [name for name in self.designs.keys()
                       if self.designs[name].base_image]
        app_images = [name for name in self.designs.keys(
        ) if not self.designs[name].base_image]
    ...
    def entry(self):
        for name in self.designs.keys():
            ...
            icon_file = os.path.join(app_path, os.path.basename(
                icon_files[0])) if len(icon_files) > 0 else ''
            entry = (
                "[Desktop Entry]\n"
                "Type=Application\n"
                "Name={0}\n"
            ...
            entry_file = os.path.join(
                desktopfile_dir, '{}.desktop'.format(name))
